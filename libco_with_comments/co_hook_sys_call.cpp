...
static rpchook_t *g_rpchook_socket_fd[ 102400 ] = { 0 }; //定义一个数组， 存放对应描述符， 描述符的值就是对应数组的下标
...
{
	int cli = accept( fd,addr,len ); //调用系统函数
	if( cli < 0 )
	{
		return cli;
	}
...
int connect(int fd, const struct sockaddr *address, socklen_t address_len)
{
    ...
    //等待描述符是否连接 循环3次 每次25s 也就是75s后 未连接返回
...
//原理同上 类似  没有被钩住则调用系统函数， 有则利用poll来读取
...
int fcntl(int fildes, int cmd, ...)
{
	...
	switch( cmd )
	{
		...
		case F_SETFL:
		{
			...
			if( 0 == ret && lp )
			{
                //这里rpchook_t 里面存入的仍然是用户设置的，如果用户设置O_NONBLOCK 并没有保留上面的O_NONBLOCK
				lp->user_flag = param;
			}
			break;
		}
