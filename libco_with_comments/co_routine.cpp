...
struct stTimeoutItem_t
{
	...
	OnPreparePfn_t pfnPrepare; //处理之前回调函数
		...
		pfn_co_routine_t pfn,void *arg )
{
	...
	if( at.stack_size & 0xFFF )  //将空间分配为4KB的整数倍
	{
		at.stack_size &= ~0xFFF;
...
{
	stCoRoutineEnv_t *env = co->env;
	stCoRoutine_t *lpCurrRoutine = env->pCallStack[ env->iCallStackSize - 1 ]; //当前执行的协程
	if( !co->cStart )
	{
        //设置协程栈区 为执行前准备
	...
	env->pCallStack[ env->iCallStackSize++ ] = co; //压栈
	co_swap( lpCurrRoutine, co ); //执行协程
...
void co_yield_env( stCoRoutineEnv_t *env )
{
	...
	stCoRoutine_t *last = env->pCallStack[ env->iCallStackSize - 2 ]; //栈顶上一个协程
	stCoRoutine_t *curr = env->pCallStack[ env->iCallStackSize - 1 ]; //当前协程

	env->iCallStackSize--; //删除栈顶协程

	co_swap( curr, last); //把当前协程切换出去
}
